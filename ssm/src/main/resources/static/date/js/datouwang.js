...
function setA(){
	...
	for(var i=0;i<arr.length;i++){
		...
		var datearr=date.split("-");
			...
			arr[i].setAttribute("onclick","javascript:setRigth("+datearr[0]+","+datearr[1]+","+datearr[2]+");javascript:stopBubble(this);javascript:aboluoSetrq();");
			}
...
//根据年月日判断是不是星期六星期天 //yue 按12算 www.datouwang.com
...
function aboluoSetrq(){
	var curra=getAclickDomDate();
	$("#workdate").val(curra);
	$.post("daily/existDate.do", {userid:$("#userid").val(),workdate:$("#workdate").val()},function(data){
		if(data>0){
			layer.confirm('当前日期的工作日报已经填写', {
				  btn: ['确定'] //按钮
				}, function(){
